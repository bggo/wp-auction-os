				...
				<div class="block-header">
					<h2 class="page-title">
						<?php /* show archive title */
							if ( is_day() ) { 
								printf( __( 'Daily Archives: %s', 'central' ), '<span>' . get_the_date() . '</span>' ); 
							} elseif ( is_month() ) {
								printf( __( 'Monthly Archives: %s', 'central' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'central' ) ) . '</span>' ); 
							} elseif ( is_year() ) {
								printf( __( 'Yearly Archives: %s', 'central' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'central' ) ) . '</span>' ); 
							} else {
								_e( 'Blog Archives', 'central' ); 
							}; ?>
					</h2><!-- .page-title -->
				</div><!-- .block-header-->
			</div><!-- .post -->
				...
				while ( have_posts() ) : the_post(); ?>
					<div <?php post_class(); ?>>
						...
						<div class="block-header">
							...
							<p class="post-date">
								<?php /* show the date publication of current post and categories */ _e('Posted on ', 'central' ); ?><a href="<?php the_permalink( get_the_ID() ); ?>" title="<?php the_title(); ?>" rel="bookmark"><span id="date"><?php the_date(); ?></span></a><?php _e( ' in ', 'central' ); the_category( ', ' ); ?>
							</p>
						</div><!-- .block-header -->
