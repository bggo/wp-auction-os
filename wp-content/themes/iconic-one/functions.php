...
function themonic_content_nav( $html_id ) {
	...
	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav id="<?php echo $html_id; ?>" class="navigation" role="navigation">
			...
			<div class="nav-previous alignleft"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Anterior', 'themonic' ) ); ?></div>
			<div class="nav-next alignright"><?php previous_posts_link( __( 'Próximo <span class="meta-nav">&rarr;</span>', 'themonic' ) ); ?></div>
...
function query_post_type($query) {
  if($query->is_main_query() && (is_category() || is_tag())) {
	...
	if(!$post_type)
	    	$post_type = array('post','ultimate-auction'); // replace cpt to your custom post type
